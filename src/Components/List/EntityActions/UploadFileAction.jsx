...
const UploadFileAction = (props) => {
    ...
    const handleButtonClick = (e) => {
        ...
        upload(`${uploadUrl ? uploadUrl : "/fileManager/uploadFile"}`, form)
            .then(data => {
                ...
                success("InfraUploadedSuccessfully")
                setProgress(false)
            }, e => {
    ...
    return (<>
        ...
        >
            <Dialog
                title="InfraUploadFile"
                content={<div
                    className={"w-full p-4 aspect-[2/1]"}>
                    <div
                        className={(files?.length === 0 ? "relative bg-slate-100 flex justify-center items-center py-20 cursor-pointer group hover:bg-slate-200 border-dashed border-2 border-slate-400 hover:border-slate-600 dark:bg-stone-900 " : "py-20")}
                        {...getRootProps()}
                    >
                        ...
                        <Fade in={!files?.length > 0}>
                            <div>
                                ...
                                <input
                                    ...
                                    className="hidden"
                                />
                                <p className="relative text-sm tracking-wide font-bold text-slate-600 group-hover:drop-shadow-sm group-hover:drop-shadow-sm">
                                    {
                                        isDragActive ?
                                            <span>{app.t(`InfraDropTheFileHere`)}</span> :
                                            <span
                                                dangerouslySetInnerHTML={{
                                                    __html: app.t(`InfraDragAndDropAFileHereOrClickToSelectFile`)
                                                }}
                                            />
                                    }
                                </p>
