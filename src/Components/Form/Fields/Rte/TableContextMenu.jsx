...
const TableContextMenu = (props) => {
    ...
    return (
        showMenu &&
        <div
            className="contextMenu w-fit h-fit fixed z-20 bg-white border border-slate-200 rounded-md p-[0.5%] flex gap-4 flex-col cursor-pointer"
            style={{ top, left }}>
            {
                menu.map(({ icon, text, action }, index) =>
                    <div
                        className="menuOption flex gap-4s hover:dark:text-black duration-150 dark:text-gray-600"
                        key={text}
