        ...
        <div className={(hasMoreRoom ? (true ? " absolute top-2.5 start-2 w-24 h-24 " : " relative ") : " relative ") + " inline-block"}>
            ...
            <span className="group" onClick={() => setUploadOpen(true)}>
                <img
                    src={url}
                    alt={alt || ""}
                    className={"w-full cursor-pointer group-hover:shadow-md group-hover:shadow-black transition-all"}
                />
                {
                    uploadUrl &&
                    <HolismIcon
                        icon={UploadIcon}
                        className={(hasMoreRoom ? (true ? " end-16" : "end-8 ") : "end-6 ") + " w-4 h-4 absolute bottom-0 cursor-pointer text-slate-900 bg-white rounded-full p-0.5 group-hover:bg-slate-900 group-hover:text-white transition-colors"}
                    />
                }
            </span>
        </div>
